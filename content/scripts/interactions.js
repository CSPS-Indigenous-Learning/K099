...
define([
	...
	function pageLoaded() {
		...
		if(currentModule === "m0" || currentModule === "m1" || currentModule === "m2" || currentModule === "m3" || currentModule === "m4"){
			if(Utils.lang === "en"){
				setTimeout(function(){
					$(".series-title a").html("A journey towards reconciliation:");
					$("#wb-sttl a span").html("Canada through a new lens");
				}, 30);
			}
			else{
				setTimeout(function(){
					$(".series-title a").html("Sur le sentier de la réconciliation :");
					$("#wb-sttl a span").html("Le Canada sous une autre perspective");
				}, 30);
			}
		}
		else{
			setTimeout(function(){
				$(".series-title a").html("[K099] " + seriesTitle);
